var PositionIndex = {
    ...
    Init: function () {
        ...
        layui.use(['form', 'table'], function () {
			...
			//初始化查询一页数据
			PositionIndex.Search(PublicComm.PageIndex, PublicComm.PageSize);
			...
			if (PrivComm.CheckAuthority('priv-posauth-set')) {
				strBtn2 += '<a class="layui-btn layui-btn-xs" lay-event="set">设置</a>';
			}
            ...
            table.on('tool(PositionTableDate)', function (obj) {
				...
				if (obj.event === 'set') {
					layer.open({
						...
						area: ['53%', '80%'], //宽高
						content: 'position/position_form.html#set',
                ...
                if (obj.event === 'view') {
                    layer.open({
                        ...
                        //title: ["职务权限", "font-size:22px;color:black"],
                        area: ['53%', '80%'], //宽高
                        content: 'position/position_form.html#view',
                        scrollbar: false,//禁用滚动条
                        btnAlign: 'd',
                    });
                }

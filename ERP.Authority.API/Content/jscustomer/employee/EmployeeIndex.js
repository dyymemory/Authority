var EmployeeIndex = {
	...
	Init: function () {
		var str = "";
		if (PrivComm.CheckAuthority('priv-empauth-copy')) {
			str += '<a href="javascript:;" id="Copy" lay-filter="Copy" class="layui-btn" style="margin-left: 42px">批量设置员工当前城市权限</a>';
		}
		$('#copyauth').html(str);
		PrivComm.Init();
		...
		layui.use(['form', 'table'], function () {
			...
			{
				EmployeeIndex.CopyCityAuthToEmp();
			});
			//初始化加载一页数据
			EmployeeIndex.Search(PublicComm.PageIndex,  PublicComm.PageSize);
	...
	CopyCityAuthToEmp: function () {		
		layui.use("layer", function () {
			...
			layer.open({
				...
				title: '复制城市权限',
				area: ['500px', '300px'], //宽高
				content: 'employee/employee_copytoemp.html',
				scrollbar: false,//禁用滚动条
				btnAlign: 'd'
			})
		});	

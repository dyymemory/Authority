...
namespace ERP.Authority.API.Controllers.V1
{
    public class PrivilegeController : BaseApiController
    {
        ...
        {
            return privbll.UpdateEmpPrivilege(modulePriv,User);
        }
        [HttpPost]
        ...
        {
            return new Priv_PositionBLL().UpdatePrivToPosition(p_Position,User);
        }
        /// <summary>
        /// 根据员工code与平台获取员工权限信息
        /// </summary>
        ...
        {
            return new Priv_PositionBLL().UpdateUplusPositionPrivilege(Priv,User);
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public ResultModel<Priv_Employee> GetPrivEmployeeByEmployee([FromBody] Priv_Employee privEmployee)
        {
            return new Priv_EmployeeBLL().GetPrivEmployeeByEmployee(privEmployee);
        }

 [HttpPost]
        public ResultModel<object> CopyDeptToEmp([FromBody] Priv_EmployeeDataNew  priv_EmployeeDataNew)
        {
            return new Priv_EmployeeBLL().CopyDeptToEmp(priv_EmployeeDataNew, User);
        }
    }
}